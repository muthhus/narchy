...
public class Memory implements Serializable, Iterable<Concept> {
    ...
    public long decisionBlock;
    public Task lastDecision;
    public boolean allowExecution = true;

    public static long randomSeed = 1;
    public static final Random randomNumber =
            //new Random(randomSeed);
            new XorShift128PlusRandom(randomSeed);
    ...
    public final Map<Task,Task> newTasks;
    ...
    public Memory(RuntimeParameters param, Bag<Concept, Term> concepts, Bag<Task<Term>, Sentence<Term>> novelTasks,
        ...
        this.newTasks = new LinkedHashMap<>();
    ...
    public void addNewTask(final Task t, final String reason) {
        newTasks.merge(t, t, (oldt, newt) -> {
            if (oldt == null)
                return newt;
            else {
                oldt.setPriority(Math.max(oldt.getPriority(), newt.getPriority()));
                oldt.setDurability(Math.max(oldt.getDurability(), newt.getPriority()));
                oldt.setQuality(Math.max(oldt.getPriority(), newt.getPriority()));
                return oldt;
            }
        });
    }
    ...
    public void processNewTasks() {
        Task[] nn = newTasks.values().stream().toArray(Task[]::new);
        newTasks.clear(); //HACK dont just clear it like this, not entirely concurrent safe



//        int taskLimit = newTasks.size(); //only process as many as are in it at the start. this will stop from infinite looping
//        Iterator<Map.Entry<Task, Task>> ii = newTasks.entrySet().iterator();
//        while (taskLimit-- > 0 && ii.hasNext()) {
//            Map.Entry<Task, Task> taskEntry = ii.next();
//            Task task = taskEntry.getValue();
//            ii.remove();
        for (Task task : nn) {

            //  logic.TASK_ADD_NEW.commit(t.getPriority());
            emit(Events.TaskAdd.class, task, "");
            output(task);
