...
public class ConceptsPanel extends NPanel implements EventObserver, Runnable {
    ...
    public static class ConceptPanel extends JPanel {
        ...
        private final BeliefTimeline beliefTime;
        private final BeliefTimeline desireTime;
    ...
    public static Color getColor(float freq, float conf, float factor) {
       ...
       // float green = freq > 0.5f ? (freq/2f) : 0f;
        ...
        float evidence = UtilityFunctions.c2w(conf);
        float positive_evidence_in_0_1 = UtilityFunctions.w2c(evidence * freq);
        float negative_evidence_in_0_1 = UtilityFunctions.w2c(evidence * (1.0f - freq));
