...
//        return true;
    ...
    public int compareTo(final AbstractTerm that) {
        ...
        if (Parameters.TERM_ELEMENT_EQUIVALENCY) {
            ...
            if (thatVar && (getClass()!=Variable.class))
                return 1;
            else if (thisVar && (that.getClass()!=Variable.class))
                return -1;

            int c = Texts.compareTo(name(), that.name());
            if ((c == 0) && thisVar && thatVar)  {
                Term as = ((Variable) this).getScopeIfNotSelf();
                Term bs = ((Variable) that).getScopeIfNotSelf();
                if (as == bs)
                    return 0;
                ...
                if (Parameters.DEBUG) {
                    ...
                    if (equal ? c != 0 : c == 0) {
                        throw new RuntimeException("invalid order: " + a + " = " + b +
                            "equals=" + a.equals(b) + ", compare(a,b)=" + a.compareTo(b) + ", compare(b,a)=" + b.compareTo(a)
                        );
                    }
                }
