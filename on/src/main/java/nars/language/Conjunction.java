...
public class Conjunction extends CompoundTerm {
    ...
    public static Term[] removeFirstInterval(Term[] arg) {
        if(arg[0] instanceof Interval) {
            ...
            System.arraycopy(arg, 1, argNew, 0, arg.length - 1);
            return argNew;
        }
        return arg;
    }
    ...
    final public static Term make(final Term[] argList, final int temporalOrder) {
        ...
        if (temporalOrder == TemporalRules.ORDER_FORWARD) {
            Term[] newArgList = removeFirstInterval(flatten(argList, TemporalRules.ORDER_FORWARD));
            if(newArgList.length == 1) {
                return newArgList[0];
            }
            return new Conjunction(newArgList, TemporalRules.ORDER_FORWARD, false);
