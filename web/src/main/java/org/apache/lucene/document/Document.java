...
public final class Document extends FasterList<IndexableField> {
     ...
     */
//  public final boolean add(IndexableField field) {
//    fields.add(field);
//  }
    ...
    public final void removeField(String name) {
        Iterator<IndexableField> it = iterator();
        while (it.hasNext()) {
    ...
    public final BytesRef getBinaryValue(String name) {
        for (IndexableField field : this) {
            final BytesRef bytes = field.binaryValue();
            if (bytes!=null && field.name().equals(name)) {
                return bytes;
            }
        }
        return null;
    ...
    public IndexableField[] getFields(String name) {
        List<IndexableField> result = new FasterList();
        int s = 0;
        for (int i = 0, thisSize = this.size(); i < thisSize; i++) {
            IndexableField field = this.get(i);
            if (field.name().equals(name)) {
                result.add(field);
                s++;
            }
        ...
        switch (s) {
            case 0:
                return IndexableField.empty;
            case 1:
                return new IndexableField[] { result.get(0) };
            default:
    ...
    public final String[] getValues(String name) {
        List<String> result = new FasterList<>();
        for (int i = 0, thisSize = this.size(); i < thisSize; i++) {
            IndexableField field = this.get(i);
            String val = field.stringValue();
            if (val != null && field.name().equals(name)) {
                result.add(val);
            }
        }
