...
import org.apache.lucene.util.filter.FilterFileSystem;
...
public class TestUtil0 {
    ...
    // used by checkReader to check some things really unrelated to the index,
    // just looking for bugs in indexreader implementations.
    protected static void checkReaderSanity(LeafReader reader) throws IOException {
    ...
    public static int nextInt(Random r, int start, int end) {
        ...
        if (end==start)
            return start;
        return r.nextInt(1+end-start)+start;
    }
    ...
    public static BigInteger nextBigInteger(Random random, int maxBytes) {
        int length = nextInt(random, 1, maxBytes);
    ...
    public static Pattern randomPattern(Random random) {
        ...
        while (true) {
            try {
                Pattern p = Pattern.compile(randomRegexpishString(random));
    ...
    public static String randomRegexpishString(Random r, int maxLength) {
        ...
        for (int i = nextInt(r, 0, maxLength); i > 0; i--) {
            if (r.nextBoolean()) {
                regexp.append(randomCharBetween(r, 'a', 'z'));
            } else {
                regexp.append(randomFrom(r, ops));
            }
        }
        return regexp.toString();
    }

    private static <X> X randomFrom(Random r, List<X> l) {
        return l.get(r.nextInt(l.size()));
    }

    private static char randomCharBetween(Random r, char a, char z) {
