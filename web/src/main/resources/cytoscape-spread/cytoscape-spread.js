...
// registers the extension on a cytoscape lib ref
            ...
            SpreadLayout.prototype.run = function () {
                ...
                if (nodes.size() <= 1) {
                    nodes.positions({
                        x: ( ( simBB.x1 + simBB.x2 ) / 2 ),
                        y: ( ( simBB.y1 + simBB.y2 ) / 2 )
                    });
                ...
                nodes.each(
                    function (i, node) {
                        ...
                        if (options.randomize) {
                            pos = {
                                x: ( simBB.x1 + (simBB.x2 - simBB.x1) * Math.random() ),
                                y: ( simBB.y1 + (simBB.y2 - simBB.y1) * Math.random() )
                            };
                        }
...
//   *references* to sites are copied locally.
            ...
            Voronoi.prototype.compute = function (sites, bbox) {
                ...
                diagram.execTime = 0; //stopTime.getTime()-startTime.getTime();
