...
//            })
            ...
            .meter( (metrics, nar) -> {
                ...
                metrics
                        .set("a pri", nar.conceptPriority(term("a"), 0.0f))
                        .set("b pri", nar.conceptPriority(term("b"), 0.0f))
                        .set("c pri", nar.conceptPriority(term("c"), 0.0f))
                        .set("<a-->b> pri", nar.conceptPriority(term("<a-->b>"), 0.0f))
                        //.set("<b-->a> pri", nar.memory.conceptPriority(term("<b-->a>")))
                        .set("<b-->c> pri", nar.conceptPriority(term("<b-->c>"), 0.0f))
                        .set("<a<->b> pri", nar.conceptPriority(term("<a<->b>"), 0.0f))
                        .set("<a<->c> pri", nar.conceptPriority(term("<a<->c>"), 0.0f))
    ...
    public static void main(String[] args) {
        ...
        Consumer<NAR> execution = n -> {
            ...
            for (String aX : x) {
                ...
                Task t = n.task(aX);
                t.budget().setPriority(pri);
                t.budget().setQuality(qua);
                t.budget().setDurability(dur);
        ...
        NARfx.run((a,b) -> {
            ...
            //d.memory.activationRate.setValue(0.1f);
            //d.trace();
