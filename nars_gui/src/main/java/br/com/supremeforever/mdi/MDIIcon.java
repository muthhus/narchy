...
public class MDIIcon extends Button {
    ...
    private final MDICanvas mdiCanvas;
...
//Removing the shadow when the mouse cursor is off
     ...
     * ******************************** BUTTON_HANDLERES
     */
    private final EventHandler<MouseEvent> handleMaximize = new EventHandler<MouseEvent>() {
        @Override
    ...
    private final EventHandler<MouseEvent> handleClose = event -> {
        removeMDIWindow();
        removeIcon();
    };
    ...
    private void removeMDIWindow() {
        MDIWindow win = mdiCanvas.getItemFromMDIContainer(getId());
        if (win != null) {
            mdiCanvas.getChildren().remove(win);
        }
    ...
    private ImageView getImageFromAssets(String imageName) throws Exception {
        if (imageName != null) {
            ...
            if (in == null)
                return new ImageView(); //HACK
            Image ii = new Image(in);
            ImageView imvClose = new ImageView(ii);
            return imvClose;
        } else {
            return new ImageView();  //HACK
        }
