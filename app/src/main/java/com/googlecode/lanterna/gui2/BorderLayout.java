...
public class BorderLayout implements LayoutManager {
    ...
    public TerminalPosition getPreferredSize(List<Component> components) {
        ...
        int preferredHeight =
                (layout.containsKey(Location.TOP) ? layout.get(Location.TOP).getPreferredSize().row : 0)
                +
                Math.max(
                    layout.containsKey(Location.LEFT) ? layout.get(Location.LEFT).getPreferredSize().row : 0,
                    Math.max(
                        layout.containsKey(Location.CENTER) ? layout.get(Location.CENTER).getPreferredSize().row : 0,
                        layout.containsKey(Location.RIGHT) ? layout.get(Location.RIGHT).getPreferredSize().row : 0))
                +
                (layout.containsKey(Location.BOTTOM) ? layout.get(Location.BOTTOM).getPreferredSize().row : 0);
    ...
    public void doLayout(TerminalPosition area, List<Component> components) {
        ...
        if(layout.containsKey(Location.TOP)) {
            ...
            topComponentHeight = Math.min(topComponent.getPreferredSize().row, availableVerticalSpace);
            topComponent.setPosition(TerminalPosition.TOP_LEFT_CORNER);
            topComponent.setSize(new TerminalPosition(availableHorizontalSpace, topComponentHeight));
        ...
        if(layout.containsKey(Location.BOTTOM)) {
            ...
            int bottomComponentHeight = Math.min(bottomComponent.getPreferredSize().row, availableVerticalSpace);
            bottomComponent.setPosition(new TerminalPosition(0, area.row - bottomComponentHeight));
            bottomComponent.setSize(new TerminalPosition(availableHorizontalSpace, bottomComponentHeight));
        ...
        if(layout.containsKey(Location.CENTER)) {
            ...
            centerComponent.setSize(new TerminalPosition(availableHorizontalSpace, availableVerticalSpace));
        }
        ...
        for(Component component: components) {
            if(!layout.values().contains(component)) {
                ...
                component.setSize(TerminalPosition.ZERO);
            }
        }
    }
    
    private static EnumMap<Location, Component> makeLookupMap(List<Component> components) {
