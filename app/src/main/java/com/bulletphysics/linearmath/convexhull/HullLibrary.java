...
public class HullLibrary {
	...
	public boolean createConvexHull(HullDesc desc, HullResult result) {
		...
		com.bulletphysics.linearmath.convexhull.PHullResult hr = new com.bulletphysics.linearmath.convexhull.PHullResult();
	...
	private boolean computeHull(int vcount, ObjectArrayList<Vector3f> vertices, com.bulletphysics.linearmath.convexhull.PHullResult result, int vlimit) {
	...
	private com.bulletphysics.linearmath.convexhull.Tri allocateTriangle(int a, int b, int c) {
		com.bulletphysics.linearmath.convexhull.Tri tr = new com.bulletphysics.linearmath.convexhull.Tri(a, b, c);
		tr.id = tris.size();
		tris.add(tr);
	...
	private void b2bfix(com.bulletphysics.linearmath.convexhull.Tri s, com.bulletphysics.linearmath.convexhull.Tri t) {
		for (int i=0; i<3; i++) {
			int i1 = (i + 1) % 3;
			int i2 = (i + 2) % 3;
			int a = s.getCoord(i1);
	...
	private void removeb2b(com.bulletphysics.linearmath.convexhull.Tri s, com.bulletphysics.linearmath.convexhull.Tri t) {
		b2bfix(s, t);
	...
	private int calchull(ObjectArrayList<Vector3f> verts, int verts_count, IntArrayList tris_out, int[] tris_count, int vlimit) {
		...
		MiscUtil.resize(tris, 0, com.bulletphysics.linearmath.convexhull.Tri.class);
	...
	private int calchullgen(ObjectArrayList<Vector3f> verts, int verts_count, int vlimit) {
		...
		com.bulletphysics.linearmath.convexhull.Int4 p = findSimplex(verts, verts_count, allow, new com.bulletphysics.linearmath.convexhull.Int4());
		if (p.x == -1) {
		...
		t0.n.set(2, 3, 1);
		com.bulletphysics.linearmath.convexhull.Tri t1 = allocateTriangle(p.getCoord(3), p.getCoord(2), p.getCoord(0));
		t1.n.set(3, 2, 0);
		com.bulletphysics.linearmath.convexhull.Tri t2 = allocateTriangle(p.getCoord(0), p.getCoord(1), p.getCoord(3));
		t2.n.set(0, 1, 3);
		com.bulletphysics.linearmath.convexhull.Tri t3 = allocateTriangle(p.getCoord(1), p.getCoord(0), p.getCoord(2));
		t3.n.set(1, 0, 2);
		...
		while (vlimit > 0 && ((te = extrudable(epsilon)) != null)) {
			com.bulletphysics.linearmath.convexhull.Int3 ti = te;
			int v = te.vmax;
			assert (v != -1);
	...
	private void extrude(com.bulletphysics.linearmath.convexhull.Tri t0, int v) {
		com.bulletphysics.linearmath.convexhull.Int3 t = new com.bulletphysics.linearmath.convexhull.Int3(t0);
		int n = tris.size();
	...
	private static boolean hasvert(com.bulletphysics.linearmath.convexhull.Int3 t, int v) {
	...
	private static void releaseHull(com.bulletphysics.linearmath.convexhull.PHullResult result) {
		if (result.indices.size() != 0) {
			result.indices.clear();
		}
