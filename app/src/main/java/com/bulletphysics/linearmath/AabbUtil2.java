			...
			for (int j = 0; j < 2; j++) {
				for (int i = 0; i != 3; ++i) {
					if ((sourceOutcode & bit) != 0) {
						float lambda = (-com.bulletphysics.linearmath.VectorUtil.getCoord(source, i) - com.bulletphysics.linearmath.VectorUtil.getCoord(aabbHalfExtent, i) * normSign) / com.bulletphysics.linearmath.VectorUtil.getCoord(r, i);
						if (lambda_enter <= lambda) {
							lambda_enter = lambda;
							hitNormal.set(0f, 0f, 0f);
							com.bulletphysics.linearmath.VectorUtil.setCoord(hitNormal, i, normSign);
						}
					}
					else if ((targetOutcode & bit) != 0) {
						float lambda = (-com.bulletphysics.linearmath.VectorUtil.getCoord(source, i) - com.bulletphysics.linearmath.VectorUtil.getCoord(aabbHalfExtent, i) * normSign) / VectorUtil.getCoord(r, i);
