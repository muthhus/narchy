			...
			for (int j = i + 1; j < numvertices; j++) {
				...
				for (int k = j + 1; k < numvertices; k++) {
					...
					for (int ww = 0; ww < 2; ww++) {
						...
						if (com.bulletphysics.linearmath.VectorUtil.lengthSquared3(planeEquation) > 0.0001f) {
							com.bulletphysics.linearmath.VectorUtil.normalize3(planeEquation);
							if (notExist(planeEquation, planeEquationsOut)) {
								planeEquation.w = -com.bulletphysics.linearmath.VectorUtil.dot3(planeEquation, N1);
		...
		for (int i = 0; i < numbrushes; i++) {
			...
			for (int j = i + 1; j < numbrushes; j++) {
				...
				for (int k = j + 1; k < numbrushes; k++) {
					...
					com.bulletphysics.linearmath.VectorUtil.cross3(n1n2, N1, N2);
