	...
	public static void diagonalize(Matrix3f mat, Matrix3f rot, float threshold, int maxSteps) {
		...
		for (int step = maxSteps; step > 0; step--) {
			...
			for (int i=0; i<3; i++) {
				...
				mrp = com.bulletphysics.linearmath.VectorUtil.getCoord(row, p);
				mrq = com.bulletphysics.linearmath.VectorUtil.getCoord(row, q);
				com.bulletphysics.linearmath.VectorUtil.setCoord(row, p, cos * mrp - sin * mrq);
