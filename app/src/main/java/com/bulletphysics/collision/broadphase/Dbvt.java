...
public class Dbvt {
	...
	public Node insert(com.bulletphysics.collision.broadphase.DbvtAabbMm box, Object data) {
	...
	public void update(Node leaf, com.bulletphysics.collision.broadphase.DbvtAabbMm volume) {
		Node root = removeleaf(this, leaf);
		if (root != null) {
			if (lkhd >= 0) {
	...
	public boolean update(Node leaf, com.bulletphysics.collision.broadphase.DbvtAabbMm volume, Vector3f velocity, float margin) {
	...
	public boolean update(Node leaf, com.bulletphysics.collision.broadphase.DbvtAabbMm volume, Vector3f velocity) {
		if (leaf.volume.Contain(volume)) {
			return false;
		}
		volume.SignedExpand(velocity);
	...
	public boolean update(Node leaf, com.bulletphysics.collision.broadphase.DbvtAabbMm volume, float margin) {
		if (leaf.volume.Contain(volume)) {
			return false;
		}
		Vector3f tmp = new Vector3f();
	...
	public void clone(com.bulletphysics.collision.broadphase.Dbvt dest) {
		clone(dest, null);
	}

	public void clone(com.bulletphysics.collision.broadphase.Dbvt dest, IClone iclone) {
		throw new UnsupportedOperationException();
	}
	...
	public static void collideTT(Node root0, Node root1, ICollide policy) {
		...
		if (root0 != null && root1 != null) {
			...
			do {
				...
				else if (com.bulletphysics.collision.broadphase.DbvtAabbMm.Intersect(p.a.volume, p.b.volume)) {
					if (p.a.isinternal()) {
						if (p.b.isinternal()) {
	...
	public static void collideTT(Node root0, Node root1, Transform xform, ICollide policy) {
		...
		if (root0 != null && root1 != null) {
			...
			do {
				...
				else if (com.bulletphysics.collision.broadphase.DbvtAabbMm.Intersect(p.a.volume, p.b.volume, xform)) {
					if (p.a.isinternal()) {
						if (p.b.isinternal()) {
							stack.add(new sStkNN(p.a.childs[0], p.b.childs[0]));
							stack.add(new sStkNN(p.a.childs[1], p.b.childs[0]));
	...
	public static void collideTV(Node root, com.bulletphysics.collision.broadphase.DbvtAabbMm volume, ICollide policy) {
		//DBVT_CHECKTYPE
		if (root != null) {
			...
			do {
				...
				if (com.bulletphysics.collision.broadphase.DbvtAabbMm.Intersect(n.volume, volume)) {
					if (n.isinternal()) {
	...
	public static void collideRAY(Node root, Vector3f origin, Vector3f direction, ICollide policy) {
		...
		if (root != null) {
			...
			do {
				...
				if (com.bulletphysics.collision.broadphase.DbvtAabbMm.Intersect(node.volume, origin, invdir, signs)) {
					if (node.isinternal()) {
	...
	private static com.bulletphysics.collision.broadphase.DbvtAabbMm merge(com.bulletphysics.collision.broadphase.DbvtAabbMm a, com.bulletphysics.collision.broadphase.DbvtAabbMm b, com.bulletphysics.collision.broadphase.DbvtAabbMm out) {
		com.bulletphysics.collision.broadphase.DbvtAabbMm.Merge(a, b, out);
		return out;
	}

	// volume+edge lengths
	private static float size(com.bulletphysics.collision.broadphase.DbvtAabbMm a) {
		Vector3f edges = a.Lengths(new Vector3f());
		return (edges.x * edges.y * edges.z +
		        edges.x + edges.y + edges.z);
	}

	private static void deletenode(com.bulletphysics.collision.broadphase.Dbvt pdbvt, Node node) {
		//btAlignedFree(pdbvt->m_free);
		pdbvt.free = node;
	}

	private static void recursedeletenode(com.bulletphysics.collision.broadphase.Dbvt pdbvt, Node node) {
		if (!node.isleaf()) {
	...
	private static Node createnode(com.bulletphysics.collision.broadphase.Dbvt pdbvt, Node parent, com.bulletphysics.collision.broadphase.DbvtAabbMm volume, Object data) {
		Node node;
		if (pdbvt.free != null) {
			node = pdbvt.free;
			pdbvt.free = null;
	...
	private static void insertleaf(com.bulletphysics.collision.broadphase.Dbvt pdbvt, Node root, Node leaf) {
		if (pdbvt.root == null) {
			pdbvt.root = leaf;
			leaf.parent = null;
		}
			...
			if (!root.isleaf()) {
				do {
					if (com.bulletphysics.collision.broadphase.DbvtAabbMm.Proximity(root.childs[0].volume, leaf.volume) <
					    com.bulletphysics.collision.broadphase.DbvtAabbMm.Proximity(root.childs[1].volume, leaf.volume)) {
						root = root.childs[0];
					}
					else {
						root = root.childs[1];
			...
			Node node = createnode(pdbvt, prev, merge(leaf.volume, root.volume, new com.bulletphysics.collision.broadphase.DbvtAabbMm()), null);
			if (prev != null) {
				...
				do {
					if (!prev.volume.Contain(node.volume)) {
						com.bulletphysics.collision.broadphase.DbvtAabbMm.Merge(prev.childs[0].volume, prev.childs[1].volume, prev.volume);
					}
					else {
						break;
					}
	...
	private static Node removeleaf(com.bulletphysics.collision.broadphase.Dbvt pdbvt, Node leaf) {
		if (leaf == pdbvt.root) {
			pdbvt.root = null;
			return null;
		}
			...
			if (prev != null) {
				...
				while (prev != null) {
					com.bulletphysics.collision.broadphase.DbvtAabbMm pb = prev.volume;
					com.bulletphysics.collision.broadphase.DbvtAabbMm.Merge(prev.childs[0].volume, prev.childs[1].volume, prev.volume);
					if (com.bulletphysics.collision.broadphase.DbvtAabbMm.NotEqual(pb, prev.volume)) {
						prev = prev.parent;
					}
					else {
						break;
	...
	private static void fetchleaves(com.bulletphysics.collision.broadphase.Dbvt pdbvt, Node root, ObjectArrayList<Node> leaves) {
		fetchleaves(pdbvt, root, leaves, -1);
	}

	private static void fetchleaves(com.bulletphysics.collision.broadphase.Dbvt pdbvt, Node root, ObjectArrayList<Node> leaves, int depth) {
	...
	private static void bottomup(com.bulletphysics.collision.broadphase.Dbvt pdbvt, ObjectArrayList<Node> leaves) {
		com.bulletphysics.collision.broadphase.DbvtAabbMm tmpVolume = new com.bulletphysics.collision.broadphase.DbvtAabbMm();
		while (leaves.size() > 1) {
	...
	private static Node topdown(com.bulletphysics.collision.broadphase.Dbvt pdbvt, ObjectArrayList<Node> leaves, int bu_treshold) {
		if (leaves.size() > 1) {
			if (leaves.size() > bu_treshold) {
				com.bulletphysics.collision.broadphase.DbvtAabbMm vol = bounds(leaves);
	...
	private static Node sort(Node n, Node[] r) {
		...
		if (p != null && p.hashCode() > n.hashCode()) {
			...
			com.bulletphysics.collision.broadphase.DbvtAabbMm.swap(p.volume, n.volume);
			return p;
		}
		return n;
	}
	...
	public static class Node {
		public final com.bulletphysics.collision.broadphase.DbvtAabbMm volume = new DbvtAabbMm();
		public Node parent;
