...
public class GhostObject extends CollisionObject {
	...
	public void convexSweepTest(ConvexShape castShape, Transform convexFromWorld, Transform convexToWorld, com.bulletphysics.collision.dispatch.CollisionWorld.ConvexResultCallback resultCallback, float allowedCcdPenetration) {
		...
		for (int i=0; i<overlappingObjects.size(); i++) {
			...
			if (resultCallback.needsCollision(collisionObject.getBroadphaseHandle())) {
				...
				if (AabbUtil2.rayAabb(convexFromWorld.origin, convexToWorld.origin, collisionObjectAabbMin, collisionObjectAabbMax, hitLambda, hitNormal)) {
					com.bulletphysics.collision.dispatch.CollisionWorld.objectQuerySingle(castShape, convexFromTrans, convexToTrans,
					                                 collisionObject,
	...
	public void rayTest(Vector3f rayFromWorld, Vector3f rayToWorld, com.bulletphysics.collision.dispatch.CollisionWorld.RayResultCallback resultCallback) {
	...
	public static com.bulletphysics.collision.dispatch.GhostObject upcast(CollisionObject colObj) {
		if (colObj.getInternalType() == CollisionObjectType.GHOST_OBJECT) {
			return (com.bulletphysics.collision.dispatch.GhostObject)colObj;
		}
