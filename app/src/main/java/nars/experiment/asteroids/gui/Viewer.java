package nars.experiment.asteroids.gui;
...
public class Viewer extends JComponent implements Observer {
    ...
    private int quality = QUALITY_MAX; /* 0 - 2 quality setting. */
...
//        }
    ...
    private void minimap(Collection<Mass> objects, Graphics2D g) {
        ...
        for (Mass m : objects) {
            if (m.isShortlived() && !(m instanceof Loot)) continue;
            at.setToTranslation(m.getX(0), m.getY(0));
            g.setColor(m.getFaction().getColor());
            if (m instanceof Ship && ((Ship) m).getCanMove()) {
                at.rotate(m.getA(0));
