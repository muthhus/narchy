...
//		map.addActor(c, "forest", "aimless");
    ...
    public void drawMap(Console c) {
        ...
        while (i >= 0 && m[i].getTurn() >= gt.getCurrentTurn() - 1 && !m[i].isShown()) {
            ...
            if (s[str].length() + m[i].show(!needRefresh()).length() + 1 < c.getSymWidth())
                s[str] = msg.show(!needRefresh()) + " " + s[str];
            else if (str < s.length - 1) {
                str++;
                if (s[str].length() + msg.show(!needRefresh()).length() + 1 < c.getSymWidth())
                    s[str] = msg.show(!needRefresh()) + " " + s[str];
            } else {
            ...
            if ((xy[0] != 0 || xy[1] != 0) && pc.getX() + xy[0] >= 0 && pc.getX() + xy[0] < map.getWidth() && pc.getY() + xy[1] >= 0 && pc.getY() + xy[1] < map.getHeight()) {
                ...
                if (a != null) {
                    ...
                    if (tl.getTimeMultiplier() != 0) {
                        map.movePlayer(xy);
                        messageItems();
                    } else message("Obstructed path");
                }
            }
        }
        showEvents();
        if (!pc.immortal && pc.getHP() <= 0)
            dialog(new DeathQuestion());
        return false;
    }
    ...
    private void showEvents() {
        ...
        for (int i = 0; i < me.length; i++) {
            if (me[i].getType() == MapEvent.T_UNDER_ATTACK) {
                ...
                if (Objects.equals(me[i].getName(), "miss")) {
                    message(who + " misses.");
                } else if (Objects.equals(me[i].getName(), "hit")) {
                    message(who + " hits you for " + me[i].getValue() + " hp.");
                } else if (Objects.equals(me[i].getName(), "you_hit")) {
                    if (c != null) who = c.getTheName(false);
                    message("You hit " + who + " for " + me[i].getValue() + " hp.");
                } else if (Objects.equals(me[i].getName(), "you_kill")) {
                    if (c != null) who = c.getTheName(false);
                    message("You've killed " + who + ".");
                } else if (Objects.equals(me[i].getName(), "death")) {
                    message("You die!");
                }
            }
        }
    ...
    private class PickupQuestion implements IGameScreen {
        ...
        public boolean getKeyEvent(KeyEvent ke) {
            ...
            return cur >= item_indexes.length;
        }
