...
public class Peer {
	...
	private File myDirectory;
 	final Map<String,Message> messageCache = new CapacityLinkedHashMap(256);
	...
	public Peer(String pathName, ClientModel model) throws IOException {
		...
		neighbors = new ConcurrentHashMap<InetSocketAddress, PeerThread>();
		downloads = new ConcurrentHashMap<InetSocketAddress, PeerThread>();
		...
		this.client = new Client(this, myPort, neighbors,
				ipAddress,
				IdGenerator.getIdServent(), downloads, model
        );

		client.start();

		this.server = new Server(this, myPort, neighbors,
				myIdGenerator, downloads,
				myDirectory, maxNodes, client);
...
//	}
	...
	public void stop() {
		...
		while (d.hasMoreElements()) {
			...
			downloads.remove(idN).close();
		}

	}

	private short newRandomPort() {
		Random r = new Random();
