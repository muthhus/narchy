...
public class SimpleBroadphase extends BroadphaseInterface {
    ...
    @Override
    public void setAabb(@NotNull BroadphaseProxy proxy, Vector3f aabbMin, Vector3f aabbMax, Dispatcher dispatcher) {
        SimpleBroadphaseProxy sbp = (SimpleBroadphaseProxy) proxy;
        sbp.min.set(aabbMin);
        sbp.max.set(aabbMax);
    }

    private static boolean aabbOverlap(SimpleBroadphaseProxy a, SimpleBroadphaseProxy b) {
        float aminX = a.min.x;
        float bmaxX = b.max.x;
