...
public class DiscreteDynamics<X> extends Dynamics<X> {
	...
	protected Islands islandManager;
	...
	protected void saveKinematicState(float timeStep) {
		...
		objects.removeIf(c-> {
			if (valid(nextID,c)) {
				Tangible body = Tangible.upcast(c);
...
//			}
	...
	@Override
	public final void addRigidBody(Tangible body) {
	...
	public final void addRigidBody(Tangible body, short group, short mask) {
		if (!body.isStaticOrKinematicObject()) {
			body.setGravity(gravity);
		}
	...
	protected void calculateSimulationIslands() {
		...
		try {
            islandManager.updateActivationState(getCollisionWorld(), getCollisionWorld().getDispatcher());
            ...
            for (i = 0; i < numConstraints; i++) {
                ...
                Tangible colObj0 = constraint.getRigidBodyA();
                Tangible colObj1 = constraint.getRigidBodyB();
