...
/*
static final public ImmutableCharObjectMap CHAR_MAP =
...
static public String munge(String name){
	...
	StringBuilder sb = new StringBuilder(0); //TODO lazily allocate on first change
	int l = name.length();
	for (int i = 0; i < l; i++) {
...
//			fn.superName = (String) RT.get(RT.meta(form.first()), Keyword.intern(null, "super-name"));
		...
		String rnext = Integer.toString(RT.nextID(), 36);

		if(RT.second(form) instanceof Symbol) {
			nm = (Symbol) RT.second(form);
			name = nm.name + "__" + rnext;
		} else {
			if(name == null)
				name = "fn__" + rnext;
		...
		List<String> prims = new FasterList();
		try
			{
			Var.pushThreadBindings(
...
//			loader.registerConstants(fn.constantsID, fn.constants.toArray());
		...
		try
			...
			fn.compile(fn.isVariadic() ? "clojure/lang/RestFn" : "clojure/lang/AFunction",
			           (numPrims == 0)?
			            null
						:prims.toArray(new String[numPrims]),
			            fn.onceOnly);
			}
		catch(IOException e)
			{
...
//	else
		...
		{
		if(!(e instanceof CompilerException))
			throw new CompilerException(SOURCE_PATH!=null ? (String) SOURCE_PATH.deref() : "", lineDeref(), columnDeref(), e);
		else
			throw (CompilerException) e;
		}
}
...
public static Object eval(Object form, ClassLoader cl) {
	boolean createdLoader = false;
	if(true)//!LOADER.isBound())
		{
		Var.pushThreadBindings(RT.map(LOADER, cl));
		createdLoader = true;
		}
	try
		{
		...
		IPersistentMap m = RT.meta(form);
		if (m!=null) {
			if (m.containsKey(RT.LINE_KEY))
				line = m.valAt(RT.LINE_KEY);
			if (m.containsKey(RT.COLUMN_KEY))
				column = m.valAt(RT.COLUMN_KEY);
		}
