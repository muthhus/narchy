		...
		if(!unchanged) {
			...
			for(int i = 0; i < bufferSize; ++i) {
				if(currentSegment == null) {
					...
					ratio = currentSegment.curvature != 1.0f ? (float) Math.pow((double) currentTime / (double) currentSegment.duration, (double) currentSegment.curvature) : (float) currentTime / (float) currentSegment.duration;
