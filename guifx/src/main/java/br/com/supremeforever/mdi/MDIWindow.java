...
import static br.com.supremeforever.mdi.MDICanvas.resourceURI;
...
//        paneTitle.styleProperty().bind(StylesCSS.mdiTitleBarStyleProperty);
        ...
        btnClose = new Button("", mdiImageResource("close.png"));
...
//        btnClose.styleProperty().bind(StylesCSS.controlButtonsStyleProperty);
        ...
        btnMinimize = new Button("", mdiImageResource("minimize.png"));
...
//        btnMinimize.styleProperty().bind(StylesCSS.controlButtonsStyleProperty);
        ...
        btnMaximize = new Button("", mdiImageResource("maximize.png"));
...
//        btnMaximize.styleProperty().bind(StylesCSS.controlButtonsStyleProperty);
    ...
    public void minimizeMdiWindow() {
        ...
        borderPane.fireEvent(new MDIEvent(
            imgLogo!=null ? imgLogo : mdiImageResource("restore.png"),
            MDIEvent.EVENT_MINIMIZED));
    }
    ...
    public void maximizeRestoreMdiWindow() {
        ...
        if (isMaximized == false) {
            ...
            isMaximized = true;
            try {
                btnMaximize.setGraphic(mdiImageResource("restore.png"));
            } catch (Exception ex) {
            ...
            isMaximized = false;
            try {
                btnMaximize.setGraphic(mdiImageResource("maximize.png"));
            } catch (Exception ex) {
                Logger.getLogger(MDIWindow.class.getName()).log(Level.SEVERE, null, ex);
            }
...
//        lblTitle.setStyle("-fx-font-weight: bold;");
    ...
    private void moveListener() {
        this.setOnMouseDragged((MouseEvent dragEvent) -> {
            if (!isMaximized) {
                ...
                if (resizeMode == resizeMode.NONE) {
                    ...
                    if (bp.getHeight() < bp.getParent().getLayoutBounds().getHeight()) {//if the panel is not biger then the window: Move
                        bp.setLayoutY(y);
                    }

                    //LEFT AND RIGHT
                    if (bp.getLayoutX() <= bp.getParent().getLayoutX()) {
                        bp.setLayoutX(bp.getParent().getLayoutX());
                    } else if ((bp.getLayoutX() + bp.getWidth()) >= bp.getParent().getLayoutBounds().getWidth()) {
                        bp.setLayoutX(bp.getParent().getLayoutBounds().getWidth() - bp.getWidth());
                    }
                    //UP AND DOWN
                    if (bp.getLayoutY() <= bp.getParent().getLayoutX()) {
                        bp.setLayoutY(bp.getParent().getLayoutY());
                    } else if ((bp.getLayoutY() + bp.getHeight()) >= bp.getParent().getLayoutBounds().getHeight()) {
                        bp.setLayoutY(bp.getParent().getLayoutBounds().getHeight() - bp.getHeight());
                    }
                } else {
                    if (!disableResize) {
                        //Resize
        ...
        onMouseMovedProperty().set((MouseEvent t) -> {
            Transform stt = localToSceneTransformProperty().getValue();
            final double scaleX = stt.getMxx();
            final double scaleY = stt.getMyy();
            final double border1 = 5;
            Bounds bil = getBoundsInLocal();
            double diffMinX = Math.abs(bil.getMinX() - t.getX());
            double diffMinY = Math.abs(bil.getMinY() - t.getY());
            double diffMaxX = Math.abs(bil.getMaxX() - t.getX());
            double diffMaxY = Math.abs(bil.getMaxY() - t.getY());
    ...
    public void placeMdiWindow(AlignPosition alignPosition) {
        Platform.runLater(() -> {
            ((MDICanvas) this.getParent()).position(this, alignPosition);
        });
    }

    public void placeMdiWindow(Point2D point) {
        Platform.runLater(() -> {
            ((MDICanvas) this.getParent()).position(this, point);
        });
    }

    public void centerMdiWindow() {
        Platform.runLater(() -> {
            ((MDICanvas) this.getParent()).centerMdiWindow(this);
        });
    }
    ...
    public static ImageView mdiImageResource(String imageName) {
        InputStream in = null;
        try {
            return new ImageView(
                    resourceURI("mdi/" + imageName).toURL().toString());
        } catch (Exception e) {
            e.printStackTrace();
            return new ImageView(); //blank
        }
