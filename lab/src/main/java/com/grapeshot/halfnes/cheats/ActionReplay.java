...
public class ActionReplay {
    ...
    private final IntObjectHashMap<Patch> patches = new IntObjectHashMap<>();
     ...
     */
    public IntObjectHashMap<Patch> getPatches() {
        return patches;
    }
    ...
    public void addMemoryPatch(Patch patch) {
        if (!patches.containsKey(patch.address)) {
            patches.put((Integer) patch.address, patch);
        }
    }
