...
//        }
    ...
    public final double[] chroma = new double[2728];
    public final double[] luma = new double[2728];
    ...
    @Override
    public BufferedImage render(int[] nespixels, int[] bgcolor, boolean dotcrawl, BufferedImage lastFrame) {

        for (int line = 0; line < 240; ++line) {
            ntsc_decode(ntsc_encode(nespixels, line * 256, bgcolor[line], dotcrawl), frame, line * frame_w);
        }

        return getImageFromArray(frame, frame_w * 8, frame_w, 224, lastFrame);
    }
