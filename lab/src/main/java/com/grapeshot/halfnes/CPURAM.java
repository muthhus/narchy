...
public class CPURAM {
    ...
    private IntObjectHashMap<Patch> patches = new IntObjectHashMap<>();
    ...
    public final int read(final int addr) {
        if (!patches.isEmpty()) {
            ...
            if (p != null && p.address == addr && p.matchesData(retval)) {
                return p.data;
            }
            return retval;
        } else {
...
//        }
    ...
    public void setPatches(IntObjectHashMap<Patch> p) {
        this.patches = p;
    }
}
