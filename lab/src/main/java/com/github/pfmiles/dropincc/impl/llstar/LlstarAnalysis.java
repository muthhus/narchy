...
public class LlstarAnalysis {
    ...
    // kleene node grules')
    private final Map<GruleType, LookAheadDfa> gruleDfaMapping = new HashMap<>();

    // the resulting kleene node to look-ahead dfa mapping
    private final Map<KleeneType, LookAheadDfa> kleenDfaMapping = new HashMap<>();

    // grules which is not a LL(*) grammar
    private final Set<GruleType> nonLLRegularGrules = new HashSet<>();

    // kleenes which is not LL-regular
    private final Set<KleeneType> nonLLRegularKleenes = new HashSet<>();
    ...
    public LlstarAnalysis(Map<GruleType, List<CAlternative>> ruleTypeToAltsOriginal, Map<KleeneType, List<EleType>> kleeneTypeToNode) {
        Map<GruleType, List<CAlternative>> ruleTypeToAlts = new HashMap<>(ruleTypeToAltsOriginal);
        ...
        Map<KleeneType, AtnState> contactPoints = new HashMap<>();
                ...
                if (alt != null && !alt.isEmpty()) {
                    ...
                    for (EleType edge : alt) {
                        if (edge instanceof TokenType || edge instanceof GruleType) {
                            ...
                            this.atn.genTransitions(curState, kleeneTypeToNode.get(edge), curState, grule, kleeneTypeToNode,
                                    contactPoints);
                            AtnState nextState = this.atn.newAtnState(grule);
        ...
        for (Map.Entry<GruleType, LookAheadDfa> e : this.gruleDfaMapping.entrySet()) {
            ...
            Set<DfaState> dests = new HashSet<>();
            for (DfaState state : dfa.getStates()) {
                dests.addAll(state.getTransitions().values());
            }
            Set<DfaState> startAndDangling = new HashSet<>(dfa.getStates());
            ...
            Set<DfaState> dangling = new HashSet<>();
            ...
            Set<Integer> allAlts = new HashSet<>();
            for (int i = 0; i < ruleTypeToAlts.get(grule).size(); i++) {
                allAlts.add(i);
            }
            Set<Integer> finaledAlts = new HashSet<>();
            for (DfaState state : dfa.getStates()) {
                if (state.isFinal())
    ...
    private static Map<GenedKleeneGruleType, AtnState> resolveContactPointMapping(Map<KleeneType, AtnState> contactPoints,
                                                                                  Map<KleeneType, GenedKleeneGruleType> kleeneGenedGruleMapping) {
        Map<GenedKleeneGruleType, AtnState> ret = new HashMap<>();
        for (Map.Entry<KleeneType, GenedKleeneGruleType> e : kleeneGenedGruleMapping.entrySet()) {
    ...
    public static boolean resolveWithPreds(DfaState state, Set<Integer> conflicts) {
        Map<Integer, Set<AtnConfig>> altToConfs = new HashMap<>();
        for (int i : conflicts) {
    ...
    public void resolveOverflow(DfaState state, GruleType grule) {
        ...
        state.getConfs().removeIf(c -> c.getAlt() != minAlt);
    ...
    public void resolveConflicts(DfaState state, GruleType grule) {
        ...
        Map<Pair<AtnState, CallStack>, Set<Integer>> ssToAlt = new HashMap<>();
        for (AtnConfig conf : state.getConfs()) {
            Pair<AtnState, CallStack> k = new Pair<>(conf.getState(), conf.getStack());
        ...
        Map<Pair<AtnState, CallStack>, Set<Integer>> conflictsKvs = new HashMap<>();
            ...
            if (edge instanceof GruleType) {
                ...
                if (depth == 1) {
                    ...
                    if (state.getRecursiveAlts().size() > 1) {
                        this.warnings.append("Likely non-LL regular grammar, recursive alts: ").append(state.getRecursiveAlts()).append(", rule: ").append(grule)
                                .append('\n');
    ...
    public LookAheadDfa createAfa(AtnState atnStartState, GruleType gruleType) throws NonLlRegularException {
        ...
        while (!work.isEmpty()) {
            ...
            // transitions
            Set<Pair<Predicate<?>, DfaState>> predTrans = new HashSet<>();
            for (AtnConfig conf : state.getResolvedConfs()) {
                predTrans.add(new Pair<>(conf.getPred(), ret.getFinalStateOfAlt(conf.getAlt())));
            }
     ...
     * 
     */
    private static boolean checkIfFinalAndReplace(DfaState state, LookAheadDfa dnet, Map<Integer, DfaState> finalsBack, boolean backup) {
