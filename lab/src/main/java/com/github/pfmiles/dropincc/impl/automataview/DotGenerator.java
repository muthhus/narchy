...
public class DotGenerator {
    ...
    private static final String graphTemp = "digraph {0} '{'\n" + "rankdir=LR;\n" + "size=\"{1},{2}\"\n" + "node [shape = doublecircle]; {3};\n"
            + "node [shape = circle];\n" + "{4}\n" + "'}'\n";

    // [start, end, edge]
    private static final String transitionTemp = "{0} -> {1} [ label = \"{2}\" ];";
    ...
    private static List<String> renderTransitionStrs(List<String[]> transitions) {
        List<String> ret = new ArrayList<>(transitions.size());
        for (String[] trans : transitions) {
            trans[2] = QUOTES.matcher(trans[2]).replaceAll("\\");
