...
public class CodiCA extends CA {
    ...
    protected void Kicking() {
        ...
        for (int iz = 0; iz < SpaceSizeZ; iz++)
            for (int iy = 0; iy < SpaceSizeY; iy++)
                for (int ix = 0; ix < SpaceSizeX; ix++) {
                    // top (+z)
                    CASpace[ix][iy][iz].IOBuf[4] = iz != SpaceSizeZ - 1 ? CASpace[ix][iy][iz + 1].IOBuf[4] : 0;
                    // north (+y)
                    CASpace[ix][iy][iz].IOBuf[2] = iy != SpaceSizeY - 1 ? CASpace[ix][iy + 1][iz].IOBuf[2] : 0;
                    // east (+x)
                    CASpace[ix][iy][iz].IOBuf[0] = ix != SpaceSizeX - 1 ? CASpace[ix + 1][iy][iz].IOBuf[0] : 0;
                }
        ...
        for (int iz = SpaceSizeZ - 1; iz >= 0; iz--)
            for (int iy = SpaceSizeY - 1; iy >= 0; iy--)
                for (int ix = SpaceSizeX - 1; ix >= 0; ix--) {
                    // bottom (-z)
                    CASpace[ix][iy][iz].IOBuf[5] = iz != 0 ? CASpace[ix][iy][iz - 1].IOBuf[5] : 0;
                    // south (-y)
                    CASpace[ix][iy][iz].IOBuf[3] = iy != 0 ? CASpace[ix][iy - 1][iz].IOBuf[3] : 0;
                    // west (-x)
                    CASpace[ix][iy][iz].IOBuf[1] = ix != 0 ? CASpace[ix - 1][iy][iz].IOBuf[1] : 0;
                }
    }
    ...
    protected void StepCA() {
        CountCAStps++;
        if (CountCAStps == MaxSteps) InitCA();
                            ...
                            if (InputSum == AXON_SIG) {// exactly one AXON_SIG
                                ...
                                for (int i = 0; i < 6; i++)
                                    if (CASpace[ix][iy][iz].IOBuf[i] == AXON)
                                        CASpace[ix][iy][iz].Gate = i;
                                for (int i = 0; i < 6; i++)
                                    CASpace[ix][iy][iz].IOBuf[i] = ((CASpace[ix][iy][iz].Chromo >>> i) & 1) != 0 ? AXON_SIG : 0;
                                break;
                            }
                            ...
                            if (InputSum == DEND_SIG) {// exactly one DEND_SIG
                                ...
                                for (int i = 0; i < 6; i++)
                                    if ((CASpace[ix][iy][iz].IOBuf[i]) != 0)
                                        CASpace[ix][iy][iz].Gate = ((i % 2) * -2) + 1 + i;
                                for (int i = 0; i < 6; i++)
                                    CASpace[ix][iy][iz].IOBuf[i] = ((CASpace[ix][iy][iz].Chromo >>> i) & 1) != 0 ? DEND_SIG : 0;
                                break;
                            }
                        ...
                        case AXON:                     // AXON
                            for (int i = 0; i < 6; i++)
                                CASpace[ix][iy][iz].IOBuf[i] =
                                        (CASpace[ix][iy][iz].IOBuf[CASpace[ix][iy][iz].Gate]);
                            CASpace[ix][iy][iz].Activ = (CASpace[ix][iy][iz].IOBuf[CASpace[ix][iy][iz].Gate]) != 0 ? 1 : 0;
                            break;
                        case DEND:                     // DENDRITE
                            InputSum =
                                    CASpace[ix][iy][iz].IOBuf[0] +
                            ...
                            for (int i = 0; i < 6; i++)
                                CASpace[ix][iy][iz].IOBuf[i] = 0;
                            CASpace[ix][iy][iz].IOBuf
                                    [CASpace[ix][iy][iz].Gate] = InputSum;
                            CASpace[ix][iy][iz].Activ = InputSum != 0 ? 1 : 0;
                            break;
                    }
                }
        Kicking();
            ...
            for (int iy = 0; iy < SpaceSizeY; iy++) {
                ...
                if (CASpace[ix][iy][iz].Type != 0) {
                    if (CASpace[ix][iy][iz].Activ != 0) {
                        ActualColor = CASpace[ix][iy][iz].Type != NEURON ? 5 : 1;
                    } else
                        switch (CASpace[ix][iy][iz].Type) {
                            case NEURON:
