...
//	MenuItem about;
    ...
    ////////////////////////////////////////////////
    public cpcman(int numGhosts, boolean showWindow) {

        this.numGhosts = numGhosts;

        if (showWindow) {
            this.window = new JFrame("CRKMAN") {
                @Override
...
//		super.dispose();
            ...
            //this.window.about.addActionListener(this);
            ...
            window.setSize(canvasWidth*zoom + insets.left + insets.right,
                    canvasHeight*zoom + insets.top + insets.bottom + 10);
...
//		addNotify();  // for updated inset information
    ...
    public void initImages() {
        if (window != null) {
            ...
            offScreen = window.createVolatileImage(cmaze.iWidth, cmaze.iHeight);
            if (offScreen == null)
            ...
            imgScore = window.createImage(150, 16);
            imgScoreG = imgScore.getGraphics();
            imgHiScore = window.createImage(150, 16);
        ...
        maze = new cmaze(window, offScreenG);
        ...
        for (int i = 0; i < numGhosts; i++) {
            ...
            if (i == 0)
                ...
                color = Color.orange;
            ghosts[i] = new cghost(window, offScreenG, maze, color);
        }

        // initialize power dot object
        powerDot = new cpowerdot(window, offScreenG, ghosts);

        // initialize pac object
        //      	pac = new cpac(this, offScreenG, maze, powerDot, ghosts);
        pac = new cpac(window, offScreenG, maze, powerDot);
...
//		invalidate();
        ...
        if (window != null && ready.compareAndSet(true, false))
            //repaint();
