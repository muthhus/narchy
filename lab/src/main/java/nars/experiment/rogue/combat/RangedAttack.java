...
import nars.experiment.rogue.map.Tile;
...
public class RangedAttack {
    ...
    public void init(int originX, int originY, int targetX, int targetY) {
        ...
        calcAzimuth();
        if (azimuth > 45 && azimuth <= 135) //W
        {
            trajectoryOrientation = EW;
            sign = -1;
        } else if ((azimuth > 135 && azimuth <= 225)) //S
        {
            trajectoryOrientation = SN;
            sign = 1;
            tan_mod = -1;
        } else if ((azimuth > 225 && azimuth <= 315)) //E
        {
            trajectoryOrientation = EW;
            sign = 1;
            tan_mod = -1;
    ...
    public void iterate() {
        if (attackCounter > 0) decAttackCounter();
        else {
            fly1Tile();
            ...
            if (getCurrentX() < 0 || getCurrentX() > map.getWidth() - 1 || getCurrentY() < 0 || getCurrentY() > map.getHeight() - 1) {
                ...
                setActive(false);
            } else if (distanceTraversed >= range)
                setActive(false);
            else {
                if (tileEffect == PtrlConstants.TE_HIT || tileEffect == PtrlConstants.TE_STOP)
                    if ((map.getTile(getCurrentX(), getCurrentY()).getHeight() > Math.random() * 10
                            && Math.max(Math.abs(originX - getCurrentX()), Math.abs(originY - getCurrentY())) > 1)
    ...
    private final double damageFactor; //damage penalty per 1 range unit (tile)
    private final int damageType;
    private final double basicTohit; //assume that we already at tile, where target stands
    private final double tohitFactor;
    private int[] effects;
