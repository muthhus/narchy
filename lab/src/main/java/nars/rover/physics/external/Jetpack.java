...
public class Jetpack {
	...
	/**
	 * How much fuel is left in this jetpack. Should always be between 0 and
	 * 100.
	 */
	...
	public Jetpack(DynamicEntity player) {
		...
		jetpackEmitter = new ParticleEmitter(player.getLayer() - 1,
				new JetpackEmitterSettings(player));
	...
	public void update(float timeStep) {
		...
		if (jetpackOn && fuel > 0.0f) {
			applyForce(timeStep);
			fuel -= isHovering ? timeStep * hoverDepleteRate : timeStep
					* depleteRate;
			fastRecharging = false;
			if (fuel <= 0.0f)
				this.disable();
	...
	private void updateSettings() {
		...
		// set offset
		settings.offset = new Vec2(rng.nextFloat() * settings.xLocationVariance
				* -0.2f, rng.nextFloat() * settings.yLocationVariance * 0.02f);
		...
		// set particle force
		settings.particleForce = new Vec2(rng.nextBoolean() ? rng.nextFloat()
				* -5.0f : rng.nextFloat() * -5.0f, rng.nextFloat() * -50.0f);
	}
	...
	private void applyForce(float timeStep) {
		...
		if (linVec.y <= maxVelocityY) {
			linVec.y += isHovering ? timeStep * hoverForce : timeStep
					* upwardForce;
	...
	public float hoverStartPercent() {
		return hoverStartPercent;
	}
}
