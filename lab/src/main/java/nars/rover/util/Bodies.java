...
public abstract class Bodies implements BodyDefCallback, FixtureDefCallback {
	 ...
	 * Default Usage
	 * 
	 * BodyDef def = SetupBody(position, body_instruction); Body ret =
	...
	public static class FixtureData {
		...
		public FixtureData(CollisionManager new_collision_manager,
				Object new_game_specific) {
			collision_manager = new_collision_manager;
			game_specific = new_game_specific;
		}

		public FixtureData(CollisionManager new_collision_manager,
				Object new_game_specific, int filterindex) {
			collision_manager = new_collision_manager;
			game_specific = new_game_specific;
	...
	public Body create(Vec2 position, Shape[] shapes, BodyType bodytype,
			BodyDefCallback body_instruction, CollisionManager colm) {
		return create(position, shapes, bodytype, body_instruction, this, colm);
	}
	public Body create(Vec2 position, Shape[] shapes, BodyType bodytype,
			FixtureDefCallback fixture_instruction, CollisionManager colm) {
		return create(position, shapes, bodytype, this, fixture_instruction,
				colm);
	}
	public Body create(Vec2 position, Shape[] shapes, BodyType bodytype,
	...
	public Body create(Vec2 position, Shape[] shapes, BodyType bodytype,
			BodyDefCallback body_instruction,
			FixtureDefCallback fixture_instruction) {
		return create(position, shapes, bodytype, body_instruction,
				fixture_instruction, null);
	}
	public Body create(Vec2 position, Shape[] shapes, BodyType bodytype,
			BodyDefCallback body_instruction) {
	...
	public Body create(Vec2 position, Shape shape, BodyType bodytype,
			BodyDefCallback body_instruction,
			FixtureDefCallback fixture_instruction, CollisionManager colm) {
		Body groundbody = world.createBody(setupBody(position, bodytype,
				body_instruction));
	...
	public Body create(Vec2 position, Shape shape, BodyType bodytype,
			FixtureDefCallback fixture_instruction, CollisionManager colm) {
		return create(position, shape, bodytype, this, fixture_instruction,
				colm);
	}
	public Body create(Vec2 position, Shape shape, BodyType bodytype,
			CollisionManager colm) {
	...
	public Body create(Vec2 position, Shape shape, BodyType bodytype,
			BodyDefCallback body_instruction,
			FixtureDefCallback fixture_instruction) {
		return create(position, shape, bodytype, body_instruction,
				fixture_instruction, null);
	}
