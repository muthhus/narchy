...
public class DecisionTreeFindBestSplitTest {
    ...
    public void testBooleanSplit() {
        ...
        dataSet.add(SimpleDataSample.data(labelColumnName, headers, TRUE_LABEL, true, true));
        dataSet.add(SimpleDataSample.data(labelColumnName, headers, FALSE_LABEL, true, false));
        dataSet.add(SimpleDataSample.data(labelColumnName, headers, FALSE_LABEL, false, true));
        dataSet.add(SimpleDataSample.data(labelColumnName, headers, FALSE_LABEL, false, false));
        
        List<Feature> features = Lists.newArrayList();
        features.add(PredicateFeature.feature("x1", true));
        features.add(PredicateFeature.feature("x2", true));
        features.add(PredicateFeature.feature("x1", false));
        features.add(PredicateFeature.feature("x2", false));
        ...
        // test splitting data
        Assert.assertEquals(TRUE_LABEL, split.get(0).get(0).value(labelColumnName).get());
        Assert.assertEquals(FALSE_LABEL, split.get(0).get(1).value(labelColumnName).get());
        Assert.assertEquals(FALSE_LABEL, split.get(1).get(0).value(labelColumnName).get());
        Assert.assertEquals(FALSE_LABEL, split.get(1).get(1).value(labelColumnName).get());
        ...
        Assert.assertEquals(TRUE_LABEL, newSplit.get(0).get(0).value(labelColumnName).get());
        Assert.assertEquals(FALSE_LABEL, newSplit.get(1).get(0).value(labelColumnName).get());
    }
