...
class GeneratedClassLoader extends ClassLoader {
	...
	Class<?> defineClass(final String binaryName, final ClassWriter cw) {
    ...
    private static void writeBytecodeToFile(final byte[] bytecode, final File file) {
    ...
    private static boolean dumpDirExists(final File dumpDir) {
        return (dumpDir.exists() && dumpDir.isDirectory()) || dumpDir.mkdirs();
    }

    private static String makeFilename(final String binaryName) {
