...
public class TrieMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    static class INode<K, V> extends INodeBase<K, V> {
        ...
        private static final BasicNode[] EMPTY_NODE = new BasicNode[]{};

        static <K, V> INode<K, V> newRootNode() {
            Gen gen = new Gen();
            CNode<K, V> cn = new CNode<K, V>(0, EMPTY_NODE, gen);
        ...
        private boolean equal(final K k1, final K k2, final TrieMap<K, V> ct) {
            return ct.equalityobj.test(k1, k2);
        }
...
//                throw new RuntimeException ("Should not happen");
        ...
        final Object rec_lookup(final K k, final int hc, int lev, INode<K, V> parent, final Gen startgen, final TrieMap<K, V> ct) {
            while (true) {
                ...
                if (m instanceof CNode) {
                    ...
                    return (tmp instanceof Option) ? tmp : null;
                }
    ...
    private static final class CNode<K, V> extends CNodeBase<K, V> {
        ...
        final MainNode<K, V> toCompressed(final TrieMap<K, V> ct, int lev, Gen gen) {
            ...
            return new CNode<K, V>(bitmap, tmparray, gen).toContracted(lev);
        }
    ...
    private static class Equiv<K> implements BiPredicate<K, K>, Serializable {
        private static final long serialVersionUID = 1L;

        @Override
        public boolean test(K k1, K k2) {
            return k1.equals(k2);
        }
    ...
    private interface Hashing<K> extends ToIntFunction<K>, Serializable {
    ...
    static class Default<K> implements Hashing<K> {
        ...
        static final Hashing instance = new Default();
    }

    public final Hashing<K> hashingobj;
    public final Equiv<K> equalityobj;
    ...
    final public TrieMap<K, V> snapshot() {
        while (true) {
            ...
            if (RDCSS_ROOT(r, expmain, r.copyToGen(new Gen(), this)))
                return new TrieMap<K, V>(r.copyToGen(new Gen(), this), hashingobj, equalityobj, readOnly);
            else {
                // return snapshot ();
                // tailrec
                continue;
    ...
    final public TrieMap<K, V> readOnlySnapshot() {
        ...
        while (true) {
            ...
            if (RDCSS_ROOT(r, expmain, r.copyToGen(new Gen(), this)))
                return new TrieMap<K, V>(r, hashingobj, equalityobj, true);
            else {
    ...
    // @inline
    int computeHash(K k) {
        return hashingobj.applyAsInt(k);
    }
...
//    }
    ...
    public Iterator<Entry<K, V>> readOnlyIterator() {
        TrieMap other = this;
        while (true) {
            if (other.nonReadOnly()) {
                other = other.readOnlySnapshot();
            } else
                return new TrieMapReadOnlyIterator<K, V>(0, other);
        }
    }
    ...
    public int size() {
        ...
        while (true) {
            if (other.nonReadOnly()) {
                ...
                return other.cachedSize();
        }
    }

    static String stringPrefix() {
        return "TrieMap";
    }
    ...
    private static class TrieMapIterator<K, V> implements Iterator<Entry<K, V>> {
        ...
        private int depth = -1;
        private Iterator<Entry<K, V>> subiter;
...
//            assert (ct.isReadOnly ());
        ...
        private List<Entry<K, V>> toList(Iterator<Entry<K, V>> it) {
            List<Entry<K, V>> list = new FasterList<Entry<K, V>>();
            while (it.hasNext()) {
                list.add(it.next());
            }
            return list;
    ...
    private static final long TrieMapSerializationEnd = 0L - 7237891413820527142L;
