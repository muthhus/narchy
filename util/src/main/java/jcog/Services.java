...
public class Services<X> {
    ...
    static final class ListenerCallQueue<L> {
        ...
        private final List<PerListenerQueue<L>> listeners =
                Collections.synchronizedList(new FasterList<PerListenerQueue<L>>());
     ...
     * {@link Services} constructor without having to close over the partially constructed
     * {@link Services} instance (i.e. avoid leaking a pointer to {@code this}).
     */
...
//        }
        this.state = new ServiceManagerState();
    ...
    /**
     * Registers a {@link Listener} to be run when this {@link Services} changes state. The
...
//    }
    ...
    @Override
    public String toString() {
        return MoreObjects.toStringHelper(Services.class)
                .add("services", Collections2.filter(services.values(), not(instanceOf(NoOpService.class))))
                .toString();
    }
     ...
     */
    private final class ServiceManagerState {
...
//                return count(RUNNING) == numberOfServices
...
//            @GuardedBy("ServiceManagerState.this.monitor")
//            public boolean isSatisfied() {
//                return count(TERMINATED) + count(FAILED) == numberOfServices;
//            }
//        }
...
//            monitor.enter();
//            try {
//                startupTimers.computeIfAbsent(service, k -> Stopwatch.createStarted());
//            } finally {
//                monitor.leave();
//            }
//        }
...
//                monitor.leave();
//            }
//            loadTimes.sort(Ordering.natural()
//                    .onResultOf(
//                            (Function<Entry<Service, Long>, Long>) input -> input.getValue()));
...
//                boolean log = !(service instanceof NoOpService);
     ...
     * that the {@link Services} functions properly even when it is managing no services.
     * <p>
