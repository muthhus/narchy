...
public class CompressHuffman {
    ...
    Weigher<ByteAry, Integer> memoryUsageWeigher = (key, value) -> key.ary.length + 32;

    Cache<ByteAry, Integer> freqList;
...
//			return hash;
    ...
    private void freqToTree(int freqDivide, boolean addAltNode) {
        ...
        if (freqDivide < 2) {
            for (Iterator<Entry<ByteAry, Integer>> it = freqList.asMap().entrySet().iterator(); it.hasNext(); ) {
                ent = it.next();
                if (ent.getValue() > 3) {
            ...
            while (trees.size() > maxSymbols) trees.poll();
            if (addAltNode)
                trees.add(new TmpNode(removedBytes / freqDivide == 0 ? 1 : removedBytes / freqDivide, new byte[0]));
        }
    }
