...
public class ByteUtil {
    ...
    public static final ThreadLocal<MessageDigest> sha3 = ThreadLocal.withInitial(() -> {
        String alg = "SHA3-256";
        MessageDigest y;
        try {
            y = MessageDigest.getInstance(alg);
        } catch (NoSuchAlgorithmException e) {
            System.err.println(alg + " digest algorithm not available");
            y = null;
            System.exit(1);
        }
        return y;
    ...
    public static byte[] sha3(byte[] x) {
        MessageDigest s = sha3.get();
        s.reset();
        return s.digest(x);
    }

    public static byte[] sha3(int x) {
        return sha3(intToBytes(x));
    }

    public static byte[] sha3(long x) {
        return sha3(longToBytes(x));
    }
...
//    }
    ...
    public static RawByteSeq wrap(byte[] data) {
        return new RawByteSeq(data);
    }
