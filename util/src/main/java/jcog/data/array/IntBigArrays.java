...
public enum IntBigArrays {
    ...
    public static long length(int[][] array) {
        ...
        return length == 0 ? 0 : BigArrays.start(length - 1) + array[length - 1].length;
    }
    ...
    public static void copy(int[][] srcArray, long srcPos, int[][] destArray, long destPos, long length) {
        if (destPos <= srcPos) {
            ...
            while (length > 0) {
                ...
                if ((srcDispl += l) == BigArrays.SEGMENT_SIZE) {
                    srcDispl = 0;
                    srcSegment++;
                }
                if ((destDispl += l) == BigArrays.SEGMENT_SIZE) {
                    destDispl = 0;
                    destSegment++;
                }
                length -= l;
            ...
            while (length > 0) {
                if (srcDispl == 0) {
                    srcDispl = BigArrays.SEGMENT_SIZE;
                    srcSegment--;
                }
                if (destDispl == 0) {
                    destDispl = BigArrays.SEGMENT_SIZE;
                    destSegment--;
                }
    ...
    public static int[][] newBigArray(long length) {
        ...
        int baseLength = (int) ((length + BigArrays.SEGMENT_MASK) / BigArrays.SEGMENT_SIZE);
        int[][] base = new int[baseLength][];
        int residual = (int) (length & BigArrays.SEGMENT_MASK);
        if (residual != 0) {
            for (int i = 0; i < baseLength - 1; i++) base[i] = new int[BigArrays.SEGMENT_SIZE];
            base[baseLength - 1] = new int[residual];
        } else for (int i = 0; i < baseLength; i++) base[i] = new int[BigArrays.SEGMENT_SIZE];
        return base;
    }
    ...
    public static int[][] wrap(int[] array) {
        ...
        if (array.length <= BigArrays.SEGMENT_SIZE) return new int[][]{array};
    ...
    public static int[][] ensureCapacity(int[][] array, long length, long preserve) {
        ...
        if (length > oldLength) {
            int valid = array.length - (array.length == 0 || array.length > 0 && array[array.length - 1].length == BigArrays.SEGMENT_SIZE ? 0 : 1);
            int baseLength = (int) ((length + BigArrays.SEGMENT_MASK) / BigArrays.SEGMENT_SIZE);
            int[][] base = Arrays.copyOf(array, baseLength);
            int residual = (int) (length & BigArrays.SEGMENT_MASK);
            if (residual != 0) {
                for (int i = valid; i < baseLength - 1; i++) base[i] = new int[BigArrays.SEGMENT_SIZE];
                base[baseLength - 1] = new int[residual];
            } else for (int i = valid; i < baseLength; i++) base[i] = new int[BigArrays.SEGMENT_SIZE];
            if (preserve - (valid * (long) BigArrays.SEGMENT_SIZE) > 0)
                copy(array, valid * (long) BigArrays.SEGMENT_SIZE, base, valid * (long) BigArrays.SEGMENT_SIZE, preserve - (valid * (long) BigArrays.SEGMENT_SIZE));
            return base;
        }
        return array;
    }
    ...
    public static void fill(int[][] array, long from, long to, int value) {
        ...
        Arrays.fill(array[fromSegment], fromDispl, BigArrays.SEGMENT_SIZE, value);
    }
