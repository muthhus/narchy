...
public class ConcurrentSkipTreeMap<K, V> extends AbstractMap<K, V>
    ...
    final void initialize() {
        ...
        Contents<K, V> contents = new Contents<>(keys, values, null, null);
        Node<K, V> node = new Node<>(contents);
        rootUpdater.set(this, new HeadNode<>(node, 0));
    ...
    private Node<K, V> buildFromSortedLeaf(Object[] keys, Object[] values,
        ...
        if (current == null) {
            ...
            Contents<K, V> contents = new Contents<>(keys, values, null, leafHead);
            Node<K, V> newLeafHead = new Node<>(contents);
            HeadNode<K, V> newHeadNode = new HeadNode<>(newLeafHead, 0);
            leafHeadUpdater.set(this, newLeafHead);
            rootUpdater.set(this, newHeadNode);
            current = newLeafHead;
        } else {
            Contents<K, V> contents = current.contents;
            Contents<K, V> newcontents = new Contents<>(keys, values, null, contents.link);
            Node<K, V> newnode = new Node<>(newcontents);
            Contents<K, V> replaceContents = new Contents<>(contents.keys, contents.values,
                    null, newnode);
    ...
    SearchResults<K, V> doGet(Object okey) {
        ...
        while (true) {
            if (-index - 1 == contents.keys.length) {
                ...
                results = new SearchResults<>(node, contents, index);
                return (results);
            }
            contents = node.contents;
            index = search(contents.keys, key);
    ...
    V doReplace(K kkey, V oldValue, V newValue) {
        ...
        while (true) {
            ...
            if (index < 0) {
                ...
                Contents<K, V> update = new Contents<>(contents.keys, newvalues,
                        null, contents.link);
    ...
    V removeFromNode(Comparable<? super K> key, Object value, SearchResults results) {
        while (true) {
            ...
            if (index < 0) {
                ...
                Contents<K, V> update = new Contents<>(newkeys, newvalues, null, contents.link);
                if (node.casContents(contents, update)) {
                    return (v);
                } else {
                    results = moveForward(node, key, index);
    ...
    abstract class Iter<T> implements Iterator<T> {
        ...
        public void remove() {
            ...
            SearchResults<K, V> results = new SearchResults<>(previousNode, previousContents, previousIndex);
    ...
    final class EntryIterator extends Iter<Entry<K, V>> {
        ...
        public Entry<K, V> next() {
            ...
            V v = nextValue;
            advance();
            return new SimpleImmutableEntry<>((K) okey, v);
        }
    }
    ...
    SimpleImmutableEntry<K, V> getNear(K key, int rel) {
        ...
        return (new SimpleImmutableEntry<>(nearkey, nearvalue));
    }
    ...
    private void traverseNonLeaf(Comparable<? super K> key, int target,
        ...
        while (true) {
            ...
            if (-index - 1 == contents.keys.length) {
                ...
                storeResults[0] = new SearchResults<>(node, contents, index);
                return;
            } else {
                SearchResults<K, V> results = new SearchResults<>(node, contents, index);
    ...
    private Contents<K, V> cleanLink(Node<K, V> node, Contents<K, V> contents) {
        while (true) {
            ...
            Contents<K, V> update = new Contents<>(contents.keys,
    ...
    private static <K, V> boolean slideToNeighbor(Node<K, V> sibling, Contents<K, V> sibContents,
        ...
        Contents<K, V> update = new Contents<>(keys, null, children, sibContents.link);
        return sibling.casContents(sibContents, update);
    }
    ...
    private static <K, V> Contents<K, V> deleteSlidedKey(Node<K, V> node, Contents<K, V> contents,
        ...
        Contents<K, V> update = new Contents<>(keys, null, children, contents.link);
        if (node.casContents(contents, update)) {
            return (update);
        } else {
            return (contents);
    ...
    private static <K, V> boolean dropChild(Node<K, V> node,
        ...
        System.arraycopy(contents.keys, index + 1, newkeys, index + 1 - 1, length - (index + 1));
        System.arraycopy(contents.children, index + 2, newchildren, index + 2 - 1, length - (index + 2));
        Contents<K, V> update = new Contents<>(newkeys,
    ...
    private Node<K, V> splitOneLevel(Comparable<? super K> x, SearchResults<K, V> results) {
        while (true) {
            ...
            Node<K, V> right = new Node<>(new Contents<>(
                    rightkeys, rightvalues, rightchildren, contents.link));
            Contents<K, V> left = new Contents<>(leftkeys, leftvalues, leftchildren, right);
    ...
    private V insertLeafLevel(K kkey,
        ...
        while (true) {
            ...
            if (index >= 0) {
                ...
                Contents<K, V> update = new Contents<>(keys,
    ...
    private V beginInsertOneLevel(K kkey, Comparable<? super K> key, V value,
        ...
        while (true) {
            ...
            if (index >= 0) {
                ...
                Contents<K, V> update = new Contents<>(newkeys, newvalues, null, contents.link);
                if (node.casContents(contents, update)) {
                    SearchResults newResults = new SearchResults<>(node, update, index);
    ...
    private void insertOneLevel(K kkey, Comparable<? super K> key,
        ...
        while (true) {
            ...
            if (index >= 0) {
                ...
                Contents<K, V> update = new Contents<>(newkeys, null, newchildren, contents.link);
                if (node.casContents(contents, update)) {
                    SearchResults newResults = new SearchResults<>(node, update, index);
    ...
    private static Object[] removeSingleItem(Object[] items, int index) {
        ...
        System.arraycopy(items, index + 1, newitems, index + 1 - 1, length - (index + 1));
        return (newitems);
    }
    ...
    private static <K, V> Node<K, V>[] removeSingleChild(
        ...
        System.arraycopy(children, index + 1, newchildren, index + 1 - 1, length - (index + 1));
        return (newchildren);
    }
    ...
    private HeadNode<K, V> increaseRootHeight(int target) {
        ...
        while (height < target) {
            ...
            Contents<K, V> contents = new Contents<>(keys, null, children, null);
            Node<K, V> newNode = new Node<>(contents);
            HeadNode<K, V> update = new HeadNode<>(newNode, height + 1);
    ...
    SearchResults<K, V> findFirst() {
        while (true) {
            ...
            if (contents.keys.length == 0) {
                ...
                return new SearchResults<>(node, contents, 0);
            }
        }
    }
        ...
        while (true) {
            ...
            if (contents.keys.length == 0) {
                ...
                if (node.casContents(contents, update)) {
                    ...
                    return (new SimpleImmutableEntry<>((K) okey, value));
                }
            }
        }
    }
    ...
    private SearchResults<K, V> findLastBuildStack(Deque<SearchResults<K, V>> stack) {
        ...
        while (contents.children != null) {
            if (contents.link != null) {
                ...
                stack.push(new SearchResults<>(node, contents, 0));
    ...
    private SearchResults<K, V> doLeafFindLast(Node<K, V> node, Contents<K, V> contents) {
        ...
        while (true) {
            if (contents.link != null) {
                ...
                else return (new SearchResults<>(prevNode, prevContents,
                        prevContents.keys.length - 1));
            } else {
                return (new SearchResults<>(node, contents, contents.keys.length - 2));
            }
        }
    }
    ...
    SearchResults<K, V> findLast() {
        Deque<SearchResults<K, V>> stack = new ArrayDeque<>();
    ...
    SearchResults<K, V> doFindPredecessor(Comparable<? super K> key,
        ...
        while (contents.children != null) {
            if (-index - 1 == contents.keys.length) {
                ...
                if (stack != null) {
                    ...
                    entry = new SearchResults<>(node, contents, index);
                    stack.push(entry);
                }
                node = contents.children[index];
            }
        ...
        SearchResults<K, V> leaf =
                new SearchResults<>(node, contents, index);
    ...
    SearchResults<K, V> leafFindPredecessor(Comparable<? super K> key, SearchResults<K, V> start) {
        ...
        while (true) {
            if (-index - 1 == contents.keys.length) {
                ...
                return new SearchResults<>(prevNode, prevContents,
                        prevContents.keys.length - 1);
            } else if (index > 0) {
                return (new SearchResults<>(node, contents, index - 1));
            } else {
                return (new SearchResults<>(node, contents, -index - 2));
            }
            contents = node.contents;
            index = search(contents.keys, key);
        }
    ...
    SearchResults<K, V> findPredecessor(Comparable<? super K> key) {
        ...
        SearchResults<K, V> predecessor = doFindPredecessor(key,
                new SearchResults<>(node, contents, index), stack);
        while ((predecessor == null) && !stack.isEmpty()) {
            ...
            predecessor = doFindPredecessor(key,
                    new SearchResults<>(node, contents, index - 1),
                    null);
        }
        if (predecessor == null) {
            node = this.leafHead;
            contents = node.contents;
            index = search(contents.keys, key);
            predecessor = new SearchResults<>(node, contents, index);
    ...
    static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
        ...
        KeySet(ConcurrentChunkedMap<E, Object> map) {
            m = map;
        }
        ...
        public NavigableSet<E> subSet(E fromElement,
                                      ...
                                      E toElement,
                                      boolean toInclusive) {
            return new ConcurrentSkipTreeSet<>
        ...
        public NavigableSet<E> headSet(E toElement, boolean inclusive) {
            return new ConcurrentSkipTreeSet<>(m.headMap(toElement, inclusive));
        }

        @Override
        public NavigableSet<E> tailSet(E fromElement, boolean inclusive) {
            return new ConcurrentSkipTreeSet<>(m.tailMap(fromElement, inclusive));
        }
    ...
    static final class SubMap<K, V> extends AbstractMap<K, V>
        ...
        abstract class SubMapIter<T> implements Iterator<T> {
            ...
            public void remove() {
                ...
                if (l == null)
                    throw new IllegalStateException();
                m.remove(l.contents.keys[l.index]);
                lastReturned = null;
            }
