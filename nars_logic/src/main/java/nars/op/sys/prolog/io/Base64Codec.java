...
public class Base64Codec  {
    ...
    void encode3bytes(byte[] src, int s, byte[] dest, int d) {
        ...
        byte p;
        dest[d++] = alphabets[(p=src[s++]) >>> 2 & MASK_6BITS];                         // 6
        dest[d++] = alphabets[(p & MASK_2BITS) << 4 | (p=src[s++]) >>> 4 & MASK_4BITS]; // 2 4
        dest[d++] = alphabets[(p & MASK_4BITS) << 2 | (p=src[s]) >>> 6 & MASK_2BITS];   //   4 2
        dest[d] = alphabets[p & MASK_6BITS];                                            //     6
        return;
    }

    public static void encode2chars(byte x, byte y, char[] dest, int d) {
        // operator precedence in descending order: >>> or <<, &, |
        byte p;
        byte[] a = Base64Codec.alphabets;
        dest[d++] = (char) a[(p=x) >>> 2 & MASK_6BITS];                         // 6
        dest[d++] = (char) a[(p & MASK_2BITS) << 4 | (p=y) >>> 4 & MASK_4BITS];   // 2 4
        dest[d] = (char) a[(p & MASK_4BITS) << 2];                                   //   4
    }

    public static void encode2bytes(byte[] src, int s, byte[] dest, int d) {
        // operator precedence in descending order: >>> or <<, &, |
        byte p;
        byte[] a = Base64Codec.alphabets;
        dest[d++] = a[(p=src[s++]) >>> 2 & MASK_6BITS];                         // 6
        dest[d++] = a[(p & MASK_2BITS) << 4 | (p=src[s]) >>> 4 & MASK_4BITS];   // 2 4
        dest[d++] = a[(p & MASK_4BITS) << 2];                                   //   4
        dest[d] = PAD;
        return;
    }
    ...
    void encode1byte(byte[] src, int s, byte[] dest, int d) {
        ...
        byte p;
        dest[d++] = alphabets[(p=src[s]) >>> 2 & MASK_6BITS];                           // 6
        dest[d++] = alphabets[(p & MASK_2BITS) << 4];                                   // 2
        dest[d++] = PAD;
        dest[d] = PAD;
        return;
    }
