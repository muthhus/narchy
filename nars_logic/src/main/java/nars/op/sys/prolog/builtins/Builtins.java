...
public class Builtins extends HashDict {
	...
	/**
	 * new_fun(F,N,T) creates a term T based on functor F with arity N and new
	 * free varables as arguments
	 */
	...
	/**
	 * When called as answer_source(X,G,R), it builds a new clause and maps it
	 * to an AnswerSource LD-resolution interpreter which will return one answer
	 * at a time of the form "the(X)" using G as initial resolvent and "no" when
	 * no more answers are available.
	 */
...
final class compute extends FunBuiltin {
	...
	public int exec(@NotNull Prog p) {
		...
		switch (op) {
			...
			default :
				IO.error("bad arithmetic operation <" + op + "> on " + x
						+ " and " + y);
				return 0;
		}
