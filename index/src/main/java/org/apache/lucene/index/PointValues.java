...
public abstract class PointValues {
    ...
    public static long size(IndexReader reader, String field) throws IOException {
        ...
        for (int i = 0, leavesSize = leaves.size(); i < leavesSize; i++) {
            PointValues values = leaves.get(i).reader().getPointValues(field);
            if (values != null) {
                size += values.size();
            }
        }
    ...
    public static byte[] getMinPackedValue(IndexReader reader, String field) throws IOException {
        byte[] minValue = null;
        List<LeafReaderContext> leaves = reader.leaves();
        for (int j = 0, leavesSize = leaves.size(); j < leavesSize; j++) {
            PointValues values = leaves.get(j).reader().getPointValues(field);
            if (values == null) {
                continue;
            }
    ...
    public static byte[] getMaxPackedValue(IndexReader reader, String field) throws IOException {
        byte[] maxValue = null;
        List<LeafReaderContext> leaves = reader.leaves();
        for (int i1 = 0, leavesSize = leaves.size(); i1 < leavesSize; i1++) {
            PointValues values = leaves.get(i1).reader().getPointValues(field);
            if (values == null) {
                continue;
            }
