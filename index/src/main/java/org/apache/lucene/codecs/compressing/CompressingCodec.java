...
public abstract class CompressingCodec extends FilterCodec {
  ...
  public static CompressingCodec randomInstance(Random random) {
    final int chunkSize = random.nextBoolean() ? TestUtil0.nextInt(random, 1, 10) : TestUtil0.nextInt(random, 1, 1 << 15);
    final int chunkDocs = random.nextBoolean() ? TestUtil0.nextInt(random, 1, 10) : TestUtil0.nextInt(random, 64, 1024);
    final int blockSize = random.nextBoolean() ? TestUtil0.nextInt(random, 1, 10) : TestUtil0.nextInt(random, 1, 1024);
  ...
  public static CompressingCodec reasonableInstance(Random random) {
    ...
    final int chunkSize = TestUtil0.nextInt(random, 1<<13, 1<<17);
    // e.g. defaults use 128 for FAST and 512 for HIGH
    final int chunkDocs = TestUtil0.nextInt(random, 1<<6, 1<<10);
    // e.g. defaults use 1024 for both cases
    final int blockSize = TestUtil0.nextInt(random, 1<<9, 1<<11);
    return randomInstance(random, chunkSize, chunkDocs, false, blockSize);
  }
  ...
  public static CompressingCodec randomInstance(Random random, boolean withSegmentSuffix) {
    return randomInstance(random, 
                          TestUtil0.nextInt(random, 1, 1 << 15), 
                          TestUtil0.nextInt(random, 64, 1024), 
                          withSegmentSuffix,
                          TestUtil0.nextInt(random, 1, 1024));
  }
